...
final tokenLock = _FilesystemTokenLock();

void setTokenLockDirectory(Directory? dir) {
  tokenLock.setLockDirectory(dir);
}
...
class _FilesystemTokenLock {
  ...
  final _mutex = _Mutex();
  Directory? lockDirOverride;

  void setLockDirectory(Directory? dir) => lockDirOverride = dir;

  Future<T> synchronized<T>(Future<T> Function() callback) {
    return _mutex.protect(() async {
      final baseDir = lockDirOverride ?? await getTemporaryDirectory();
